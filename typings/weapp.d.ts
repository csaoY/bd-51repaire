...
declare interface IPage {
    ...
    onShareAppMessage?: () => swan.ShareOptions;
...
declare namespace swan.{
    ...
    /**
     * 发起网络请求。`swan.request`发起的是https请求。**一个微信小程序，同时只能有5个网络请求连接**。
     */
    ...
    /**
     * 将本地资源上传到开发者服务器。
     * 如页面通过 [swan.chooseImage](#swan.chooseImage) 等接口获取到一个本地资源的临时文件路径后，可通过此接口将本地资源上传到指定服务器。
    ...
    /**
     * 通过 WebSocket 连接发送数据，需要先 [swan.connectSocket](#swan.connectSocket)，并在 [swan.onSocketOpen](#swan.onSocketOpen) 回调之后才能发送。
     */
    ...
    /**
     * 开始录音。当主动调用 `swan.stopRecord`，或者录音超过1分钟时自动结束录音，返回录音文件的临时文件路径。
     */
    ...
    /**
     * 暂停正在播放的语音。
     * 再次调用swan.playVoice播放同一个文件时，会从暂停处开始播放。如果想从头开始播放，需要先调用 swan.stopVoice。
     */
    ...
    export interface DrawCanvasOptions {
        ...
        /**
         * 绘图动作数组，由 swan.createContext 创建的 context，调用 getActions 方法导出绘图动作数组。
         */
        actions: any[];
