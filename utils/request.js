...
console.log(
	'说明：  欢迎使用 request ，request 封装了微信小程序的 swan.request 、 swan.uploadFile 、swan.downloadFile 方法，' +
...
module.exports = {
	...
	completeHandler: function (options) {
		return function (res) {
			...
			setTimeout(function () {
				if (options.isShowLoading) swan.hideToast();
				delete REQUEST_ID[options.requestId];
			}, options.delay);
	...
	request: function (options, requestId) {
		...
		if (options.isShowLoading)
			swan.showToast({ icon: "loading", title: options.loadingText, duration: 10000 });
	...
	upload: function (url, filePath, name, data, callback, callbackObj, options, requestId) {
		...
		swan.showToast({ title: options.loadingText, icon: 'loading', duration: 10000 });
		swan.uploadFile(options);
